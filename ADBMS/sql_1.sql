...
-- displaying email and phone numbers of all employees

SELECT EMAIL,PHONE_NUMBER FROM EMPLOYEES;

-- Displaying data of employees whose lastname is FAY
SELECT * FROM EMPLOYEES WHERE LAST_NAME='Fay';

-- displaying the hire date of employees whose last name is "grant" or "whalen"
SELECT HIRE_DATE FROM EMPLOYEES WHERE LAST_NAME IN("Grant","Whalen");

-- Displaying all employees who work for department 8
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID=8;

-- Displaying all departments in descneding order
SELECT * FROM DEPT ORDER BY DEPARTMENT_ID DESC;

-- Displaying all employees whose last name starts with 'K'
SELECT * FROM EMPLOYEES WHERE LAST_NAME LIKE "K%";

-- Displaying all employees whose hire date between 1995 and 1997
SELECT * FROM EMPLOYEES WHERE HIRE_DATE BETWEEN "1995-01-01" AND "1997-12-31";

-- Displaying jobs maximum salary is less than 5000
SELECT * FROM JOBS WHERE MAX_SALARY < 5000;

-- Display email address in lowercase
SELECT LOWER(EMAIL) AS LOWERCASE_EMAIL FROM EMPLOYEES;

-- Displaying name of employees who where hired in 1995
SELECT FIRST_NAME,LAST_NAME FROM EMPLOYEES WHERE HIRE_DATE BETWEEN "1995-01-01" AND "1995-12-31";

-- to insert an employee in dept 11
INSERT INTO EMPLOYEES (employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) values(209,"Paul","Newton",'paul.newton@sqltutorial.org','518.127.9362','1995-05-18',1,8300.00,205,11);

-- Deleting shipping department 
